...
def main() -> None:
    ...
    print(f"Loaded daily sample: {len(pre.hydro):,} rows across "
    ...
    print(f"Mean inflow: {stats.mean:,.0f} Cusecs | "
    ...
    print(f"Annual-mean trend: {mk.trend} (p={mk.p_value:.3g}), "
    ...
    ts = plotting.timeseries_static(
        pre.hydro, value_col=ht.COL_FLOW_CUSECS,
        title="Tarbela Daily Inflow", y_label="Inflow (Cusecs)")
    ts.savefig(OUT / "timeseries.png", dpi=150, bbox_inches="tight")

    fdc = plotting.flow_duration_curve_static(
        pre.hydro[ht.COL_FLOW_CUSECS], unit_label="Cusecs",
    ...
    print(f"Plots written to {OUT}/ (timeseries.png, flow_duration.png, "
          "timeseries.html)")
