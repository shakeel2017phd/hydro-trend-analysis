...
def test_reads_and_normalises_headers(tmp_path):
    p = _write_csv(
        tmp_path,
        "d.csv",
        pd.DataFrame({"obs": ["02/04/2000", "01/04/2000"], "flow": [200.0, 100.0]}),
    )
...
def test_value_scale(tmp_path):
    p = _write_csv(
        tmp_path, "k.csv", pd.DataFrame({"Date": ["01/04/2000"], "v": [100.0]})
    )
...
def test_daily_date_formats(tmp_path, fmt, text, expected):
    p = _write_csv(
        tmp_path, "f.csv", pd.DataFrame({"Date": [text], "inflow_cusec": [1]})
    )
...
def test_dekad_compact(tmp_path):
    p = _write_csv(
        tmp_path,
        "c.csv",
        pd.DataFrame({"Date": ["2000Apr1", "2000Jun2"], "inflow_cusec": [1, 2]}),
    )
